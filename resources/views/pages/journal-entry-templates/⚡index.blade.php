...
new #[Title('Journal entry templates')] class extends Component {
    use WithPagination;
...
<section class="w-full">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <flux:heading size="xl" level="1">{{ __('Journal entry templates') }}</flux:heading>
            <flux:subheading>{{ __('Reusable sets of journal lines to auto-fill new entries.') }}</flux:subheading>
        </div>

        <flux:button variant="primary" icon="plus" :href="route('journal-entry-templates.create', ['company' => $company->slug])" wire:navigate data-test="new-journal-entry-template-button">
            {{ __('New template') }}
        </flux:button>
    </div>
    ...
    <div class="space-y-3 lg:hidden">
        @forelse ($this->templates as $template)
            <div class="rounded-lg border border-border p-4" data-test="journal-entry-template-card">
                <div class="flex items-center justify-between gap-2">
                    <a href="{{ route('journal-entry-templates.edit', ['company' => $company->slug, 'journalEntryTemplate' => $template->id]) }}" wire:navigate class="font-medium underline">
                        {{ $template->name }}
                    </a>
    ...
    <div class="hidden overflow-x-auto rounded-lg border border-border lg:block">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-border">
                @forelse ($this->templates as $template)
                    <tr data-test="journal-entry-template-row" class="hover:bg-muted">
                        <td class="px-4 py-2">
                            <a href="{{ route('journal-entry-templates.edit', ['company' => $company->slug, 'journalEntryTemplate' => $template->id]) }}" wire:navigate class="underline">
                                {{ $template->name }}
                            </a>
                        </td>
                        ...
                        <td class="px-4 py-2 text-right">
                            <flux:button variant="ghost" size="sm" icon="trash" wire:click="delete({{ $template->id }})" wire:confirm="{{ __('Delete this template?') }}" data-test="delete-journal-entry-template" />
                        </td>
                    </tr>
                @empty
