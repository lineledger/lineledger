...
new #[Title('Vendor Contact List')] class extends Component {
    use EmailsReport;
    ...
    {
        return 'reports.vendor-contact-list';
    }
    ...
    public function rows(): array
    {
        ...
        return Contact::query()
            ...
            ->map(fn (Contact $contact) => [
                ...
                'balance' => (int) $contact->ap_balance_cents,
            ])
            ->all();
    }
    ...
    public function exportXlsx()
    {
        return app(XlsxExporter::class)->listTable(
            ...
            'Vendor Contacts',
            $this->effectiveTitle('Vendor Contact List'),
            $this->company,
            [],
            ['Name', 'Company', 'Email', 'Phone', 'Billing address', 'Terms', 'Currency', 'Open balance'],
            collect($this->rows)->map(fn (array $row) => [
    ...
    public function exportPdf()
    {
        return app(PdfExporter::class)->download('pdf.reports.list-table', [
            ...
            'title' => $this->effectiveTitle('Vendor Contact List'),
            'period' => null,
            'headers' => [
            ...
            'emptyMessage' => 'No vendors to list.',
...
<section class="w-full">
    <x-reports.control-bar
        :title="$this->effectiveTitle(__('Vendor Contact List'))"
        :subtitle="__('Vendor names, contact details, terms, and open balances.')"
        mode="none"
            ...
            <tbody class="divide-y divide-border">
                @forelse ($this->rows as $row)
                    <tr data-test="contact-list-row">
                        <td class="px-4 py-2">
                            <a
                                href="{{ route('reports.contact-statement', ['company' => $company->slug, 'contact' => $row['id'], 'kind' => 'ap']) }}"
                                class="underline"
                ...
                @empty
                    <tr><td colspan="8" class="px-4 py-8 text-center text-muted-foreground">{{ __('No vendors to list.') }}</td></tr>
                @endforelse
            </tbody>
            @if (! empty($this->rows))
