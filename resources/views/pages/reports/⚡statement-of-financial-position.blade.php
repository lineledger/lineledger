...
new #[Title('Statement of Financial Position')] class extends Component {
    ...
    {
        return 'reports.statement-of-financial-position';
    }
    ...
    public function report(): array
    {
        ...
        if (! $priorEarningsApplied && ($priorEarnings !== 0 || $priorEarningsCompare !== 0)) {
            ...
            $groups['equity'][$reKey] ??= ['label' => $labels->retainedEarnings(), 'rows' => []];
            $groups['equity'][$reKey]['rows'][] = [
                'id' => null,
                'name' => $labels->retainedEarningsPriorRow(),
                'code' => '',
                'balance' => $priorEarnings,
                'prior' => $priorEarningsCompare,
                'section_id' => null,
    ...
    public function exportCsv()
    {
        ...
        $emit = function (string $section, int $sectionTotal, int $priorSectionTotal) use (&$rows, $r, $chg, $pct) {
            $rows->push([strtoupper(str_replace('_', ' ', $section))]);
            ...
            $totalRow = ['', 'Total '.ucwords(str_replace('_', ' ', $section)), '', CsvExporter::cents($sectionTotal)];
            if ($this->showComparison) {
        ...
        $emit('equity', $r['total_equity'], $r['prior_total_equity']);

        $niRow = ['', 'Excess (deficiency) of revenue over expenses', '', CsvExporter::cents($r['net_income_ytd'])];
        $leRow = ['', 'TOTAL LIABILITIES & NET ASSETS', '', CsvExporter::cents($r['total_le'])];
        if ($this->showComparison) {
        ...
        return app(CsvExporter::class)->stream(
            "statement-of-financial-position-{$this->asOf}.csv",
            $headers,
            $rows,
        );
    }
    ...
    public function exportXlsx()
    {
        return app(XlsxExporter::class)->balanceSheet(
            "statement-of-financial-position-{$this->asOf}.xlsx",
            $this->company,
            $this->report,
            $this->asOf,
            $this->showComparison,
    ...
    public function exportPdf()
    {
        return app(PdfExporter::class)->download('pdf.reports.balance-sheet', [
            ...
            'title' => $this->effectiveTitle('Statement of Financial Position'),
            'labels' => StatementLabels::for($this->company),
        ], "statement-of-financial-position-{$this->asOf}.pdf");
    }
}; ?>
...
<section class="w-full">
    <x-reports.control-bar
        :title="$this->effectiveTitle(__('Statement of Financial Position'))"
        :subtitle="$company->name.' · '.__('as of').' '.$asOf.$this->comparisonNote"
        mode="single"
        :comparison="true"
    ...
    <div class="grid grid-cols-1 gap-6 {{ $showComparison ? '' : 'md:grid-cols-2' }}">
                ...
                <div class="border-t border-border bg-muted px-4 py-2">
                    @if ($showComparison)
                        <table class="w-full table-fixed text-base font-semibold">{!! $cmpCols !!}
                            <tr>
                                ...
                                <td class="w-24 text-right font-mono" data-test="bs-total-{{ strtolower($sec['title']) }}">{{ number_format($sec['total'] / 100, 2) }}</td>
                                <td class="w-24 text-right font-mono text-muted-foreground">{{ number_format($sec['priorTotal'] / 100, 2) }}</td>
                    ...
                    @else
                        <div class="flex justify-between text-base font-semibold">
                            ...
                            <span class="font-mono" data-test="bs-total-{{ strtolower($sec['title']) }}">{{ number_format($sec['total'] / 100, 2) }}</span>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="rounded-lg border border-border {{ $showComparison ? '' : 'md:col-start-2' }}">
            <div class="border-b border-border bg-muted px-4 py-2">
                <flux:heading>{{ __('Net Assets') }}</flux:heading>
            </div>
            <div class="p-4">
                @if ($showComparison)
                    <table class="mb-2 w-full table-fixed text-xs uppercase tracking-wide text-muted-foreground">{!! $cmpCols !!}
                ...
                <div class="mt-3 border-t border-border pt-2">
                    <table class="w-full text-sm {{ $showComparison ? 'table-fixed' : '' }}">@if ($showComparison){!! $cmpCols !!}@endif
                        <tr>
                            <td class="py-1 pl-3 italic">{{ __('Excess (deficiency) of revenue over expenses') }}</td>
                            <td class="w-24 py-1 text-right font-mono" data-test="bs-net-income">{{ number_format($this->report['net_income_ytd'] / 100, 2) }}</td>
                            @if ($showComparison)
                                <td class="w-24 py-1 text-right font-mono text-muted-foreground">{{ number_format($this->report['prior_net_income_ytd'] / 100, 2) }}</td>
            ...
            <div class="border-t border-border bg-muted px-4 py-2">
                @if ($showComparison)
                    <table class="w-full table-fixed text-base font-semibold">{!! $cmpCols !!}
                        <tr>
                            <td>{{ __('Total Liabilities & Net Assets') }}</td>
                            <td class="w-24 text-right font-mono" data-test="bs-total-le">{{ number_format($this->report['total_le'] / 100, 2) }}</td>
                            <td class="w-24 text-right font-mono text-muted-foreground">{{ number_format($this->report['prior_total_le'] / 100, 2) }}</td>
                ...
                @else
                    <div class="flex justify-between text-base font-semibold">
                        <span>{{ __('Total Liabilities & Net Assets') }}</span>
                        <span class="font-mono" data-test="bs-total-le">{{ number_format($this->report['total_le'] / 100, 2) }}</span>
                    </div>
                @endif
                @if ($this->report['total_assets'] !== $this->report['total_le'])
                    <flux:text class="mt-2 text-red-600">{{ __('Statement is out of balance — difference') }} {{ number_format(abs($this->report['total_assets'] - $this->report['total_le']) / 100, 2) }}</flux:text>
                @endif
            </div>
        </div>
    </div>
