...
new #[Title('Purchases by Vendor')] class extends Component {
    ...
    {
        return Contact::query()->where('is_vendor', true)->where('is_active', true)->orderBy('display_name')->get(['id', 'display_name']);
    }
    ...
    {
        if ($this->showComparison()) {
            $headers = ['Vendor', 'Purchases', 'Prior', 'Change', '% Change'];
        ...
        return app(CsvExporter::class)->stream("purchases-by-vendor-{$this->startDate}-{$this->endDate}.csv", $headers, $rows);
    }
    ...
    public function exportXlsx()
    {
        ...
        return app(XlsxExporter::class)->listTable(
            "purchases-by-vendor-{$this->startDate}-{$this->endDate}.xlsx",
            'Purchases by Vendor',
            $this->effectiveTitle('Purchases by Vendor'),
            $this->company,
    ...
    public function exportPdf()
    {
        ...
        if ($this->showComparison()) {
            $pct = fn (?float $p): string => $p === null ? '—' : number_format($p, 1).'%';
            $priorTotal = (int) $this->rows->sum(fn (ComparisonRow $r): int => $r->priorAmountCents);
            $headers = [['label' => 'Vendor'], ['label' => 'Purchases', 'num' => true], ['label' => 'Prior', 'num' => true], ['label' => 'Change', 'num' => true], ['label' => '% Change', 'num' => true]];
            ...
            $headers = [['label' => 'Vendor'], ['label' => 'Purchases', 'num' => true]];
        ...
        return app(PdfExporter::class)->download('pdf.reports.list-table', [
            ...
            'title' => $this->effectiveTitle('Purchases by Vendor'),
            'period' => "{$this->startDate} to {$this->endDate}".$this->comparisonNote(),
            'headers' => $headers,
            'rows' => $rows,
            'totals' => $totals,
            'emptyMessage' => 'No purchases in this period.',
        ], "purchases-by-vendor-{$this->startDate}-{$this->endDate}.pdf");
    }
}; ?>

<section class="w-full">
    <x-reports.control-bar
        :title="$this->effectiveTitle(__('Purchases by Vendor'))"
        ...
        :title-editable="true"
    >
        <flux:select wire:model.live="contactId" :label="__('Vendor')" class="max-w-[180px]" data-test="filter-contact">
