...
new #[Title('AR Aging')] class extends Component
{
    use EmailsReport;
    ...
    /**
     * The classic per-customer aging summary, reconciled to the AR control
    ...
    /**
     * Detail view: every open invoice individually, grouped by bucket, plus
     * the Adjustments section keeping the grand total tied to the GL.
     *
    ...
    public function exportXlsx()
    {
        if ($this->view === 'detail') {
            return app(XlsxExporter::class)->agingDetail(
                "ar-aging-detail-{$this->asOf}.xlsx",
                'AR Aging Detail',
                'Customer',
                'Invoice',
                $this->company,
    ...
    public function exportPdf()
    {
        if ($this->view === 'detail') {
            return app(PdfExporter::class)->download('pdf.reports.aging-detail', [
                ...
                'title' => $this->effectiveTitle('AR Aging Detail'),
                'entityLabel' => 'Customer',
                'docLabel' => 'Invoice',
                'emptyMessage' => 'No open invoices as of this date.',
                'report' => $this->detailReport,
                'asOf' => $this->asOf,
            ], "ar-aging-detail-{$this->asOf}.pdf");
        }

        return app(PdfExporter::class)->download('pdf.reports.aging', [
            'company' => $this->company,
            'title' => $this->effectiveTitle('AR Aging'),
            'entityLabel' => 'Customer',
            'emptyMessage' => 'No open invoices as of this date.',
            'report' => $this->report,
            'asOf' => $this->asOf,
        ], "ar-aging-{$this->asOf}.pdf");
    }
}; ?>

<section class="w-full">
    <x-reports.control-bar
        :title="$this->effectiveTitle($view === 'detail' ? __('AR Aging Detail') : __('AR Aging'))"
        :subtitle="$view === 'detail' ? __('Every open invoice by age.') : __('Open customer balances by age.')"
        mode="single"
        :exports="['xlsx', 'pdf']"
        ...
        <div class="overflow-x-auto rounded-lg border border-border">
            <table class="w-full text-sm">
                <thead class="bg-muted">
                    <tr>
                        <th class="px-4 py-2 text-left"><x-sort-header field="doc_no" :current-field="$sortField" :current-dir="$sortDir" :label="__('Invoice #')" /></th>
                ...
                <tbody class="divide-y divide-border">
                    @if (! $this->detailHasRows)
                        <tr><td colspan="6" class="px-4 py-8 text-center text-muted-foreground">{{ __('No open invoices as of this date.') }}</td></tr>
                    @endif
                        ...
                        @if (! empty($bucket['rows']))
                            ...
                            @foreach ($bucket['rows'] as $row)
                                <tr data-test="aging-detail-row">
                                    <td class="px-4 py-2">
                                        <a href="{{ route('invoices.show', ['company' => $company->slug, 'invoice' => $row['doc_id']]) }}" class="underline" wire:navigate>{{ $row['doc_no'] }}</a>
                                    </td>
                                    <td class="px-4 py-2">
                                        <a href="{{ route('reports.contact-statement', ['company' => $company->slug, 'contact' => $row['contact_id'], 'kind' => 'ar']) }}" class="underline">{{ $row['name'] }}</a>
                                    </td>
                            ...
                            <tr data-test="aging-detail-adjustment-row">
                                ...
                                <td class="px-4 py-2" colspan="4">
                                    @if ($adjustment['contact_id'] > 0)
                                        <a href="{{ route('reports.contact-statement', ['company' => $company->slug, 'contact' => $adjustment['contact_id'], 'kind' => 'ar']) }}" class="underline">{{ $adjustment['name'] }}</a>
                                    @else
                                        <a href="{{ route('reports.unattributed-ar', ['company' => $company->slug]) }}" class="text-muted-foreground underline">{{ $adjustment['name'] }}</a>
                                    @endif
                                </td>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-border">
                @forelse ($this->report['rows'] as $row)
                    @php($drillHref = $row['contact_id'] > 0
                        ? route('reports.contact-statement', ['company' => $company->slug, 'contact' => $row['contact_id'], 'kind' => 'ar'])
                        : route('reports.unattributed-ar', ['company' => $company->slug]))
                    <tr data-test="aging-row">
                        <td class="px-4 py-2">
                            @if ($row['contact_id'] > 0)
                                <a href="{{ $drillHref }}" class="underline" data-test="aging-contact-link">{{ $row['name'] }}</a>
                            @else
                                <a href="{{ $drillHref }}" class="text-muted-foreground underline" data-test="unattributed-link">{{ $row['name'] }}</a>
                            @endif
                        </td>
                        @foreach (array_keys($this->columnRegistry()) as $bucketKey)
                            @if ($this->columnVisible($bucketKey))
                                <td class="px-4 py-2 text-right font-mono">
                                    @if ($row[$bucketKey] != 0)
                                        <a href="{{ $drillHref }}" class="hover:underline" data-test="aging-cell-link">{{ number_format($row[$bucketKey] / 100, 2) }}</a>
                                    @else
                                        {{ number_format($row[$bucketKey] / 100, 2) }}
                                    @endif
                                </td>
                            @endif
                        @endforeach
                        <td class="px-4 py-2 text-right font-mono font-semibold">
                            <a href="{{ $drillHref }}" class="hover:underline">{{ number_format($row['total'] / 100, 2) }}</a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="{{ $this->visibleColumnCount(fixed: 2) }}" class="px-4 py-8 text-center text-muted-foreground">{{ __('No open invoices as of this date.') }}</td></tr>
                @endforelse
            </tbody>
