...
new #[Title('Sales by Item')] class extends Component {
    ...
    public function exportCsv()
    {
        ...
        if ($this->showComparison()) {
            $headers = ['Item', 'Qty', 'Prior Qty', 'Qty Change', 'Sales', 'Prior', 'Change', '% Change'];
        ...
        return app(CsvExporter::class)->stream("sales-by-item-{$this->startDate}-{$this->endDate}.csv", $headers, $rows);
    }
    ...
    public function exportXlsx()
    {
        ...
        return app(XlsxExporter::class)->listTable(
            "sales-by-item-{$this->startDate}-{$this->endDate}.xlsx",
            'Sales by Item',
            $this->effectiveTitle('Sales by Item'),
            $this->company,
    ...
    public function exportPdf()
    {
        ...
        if ($this->showComparison()) {
            $pct = fn (?float $p): string => $p === null ? '—' : number_format($p, 1).'%';
            $priorTotal = (int) $this->rows->sum(fn (ComparisonRow $r): int => $r->priorAmountCents);
            $headers = [['label' => 'Item'], ['label' => 'Qty', 'num' => true], ['label' => 'Prior Qty', 'num' => true], ['label' => 'Qty Change', 'num' => true], ['label' => 'Sales', 'num' => true], ['label' => 'Prior', 'num' => true], ['label' => 'Change', 'num' => true], ['label' => '% Change', 'num' => true]];
            ...
            $headers = [['label' => 'Item'], ['label' => 'Qty', 'num' => true], ['label' => 'Sales', 'num' => true]];
        ...
        return app(PdfExporter::class)->download('pdf.reports.list-table', [
            ...
            'title' => $this->effectiveTitle('Sales by Item'),
            'period' => "{$this->startDate} to {$this->endDate}".$this->comparisonNote(),
            'headers' => $headers,
            'rows' => $rows,
            'totals' => $totals,
            'emptyMessage' => 'No sales in this period.',
        ], "sales-by-item-{$this->startDate}-{$this->endDate}.pdf");
    }
}; ?>

<section class="w-full">
    <x-reports.control-bar
        :title="$this->effectiveTitle(__('Sales by Item'))"
