...
new #[Title('Revenu Québec — Payroll remittance')] class extends Component {
    ...
    public function summary(): array
    {
        ...
        return app(QuebecRemittanceCalculator::class)->summary($this->company, $p['start'], $p['end']);
    }
    ...
    public function rows(): array
    {
        ...
        return app(QuebecRemittanceCalculator::class)->rows($this->company, $p['start'], $p['end']);
    }
    ...
    public function recorded(): ?PayrollRemittance
    {
        return PayrollRemittance::query()
            ->where('agency', RemittanceAgency::RevenuQuebec->value)
    ...
    public function record(RecordPayrollRemittance $action): void
    {
        ...
        try {
            $action->handle([
                'agency' => RemittanceAgency::RevenuQuebec->value,
    ...
    {
        return 'revenu-quebec-'.$this->selectedPeriod()['key'];
    }
    ...
    public function exportCsv()
    {
        $rows = collect($this->rows)->map(fn ($r) => [
            ...
            CsvExporter::cents($r['quebec_gross_cents']), CsvExporter::cents($r['qpp_cents']),
            CsvExporter::cents($r['qpip_cents']), CsvExporter::cents($r['quebec_tax_cents']),
            CsvExporter::cents($r['levies_cents']), CsvExporter::cents($r['remittance_cents']),
        ]);

        $s = $this->summary;
        $rows->push(['TOTAL', '', $s['employee_count'], CsvExporter::cents($s['quebec_gross_cents']),
            CsvExporter::cents($s['total_qpp_cents']), CsvExporter::cents($s['total_qpip_cents']),
            CsvExporter::cents($s['quebec_tax_cents']), CsvExporter::cents($s['qhsf_cents'] + $s['cnesst_cents']),
            CsvExporter::cents($s['remittance_due_cents'])]);

        return app(CsvExporter::class)->stream(
            $this->fileStem().'.csv',
            ['Run #', 'Pay date', 'Employees', 'Quebec gross', 'QPP', 'QPIP', 'Quebec tax', 'QHSF + CNESST', 'Remittance'],
            $rows,
        );
    }
    ...
    {
        return app(PdfExporter::class)->download('pdf.reports.payroll-remittance-revenu-quebec', [
...
<section class="w-full">
    <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1">{{ __('Revenu Québec — Payroll remittance') }}</flux:heading>
            <flux:subheading>{{ __('Quebec source deductions and employer contributions to remit (TPZ-1015.R.14).') }}</flux:subheading>
        </div>
    ...
    {{-- Revenu Québec headline figures --}}
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-lg border border-border p-5">
            <div class="text-sm text-muted-foreground">{{ __('Quebec income tax withheld') }}</div>
            <div class="font-mono text-2xl font-semibold">{{ $this->money($s['quebec_tax_cents']) }}</div>
        </div>
        <div class="rounded-lg border border-border p-5">
            <div class="text-sm text-muted-foreground">{{ __('Total QPP (employee + employer)') }}</div>
            <div class="font-mono text-2xl font-semibold">{{ $this->money($s['total_qpp_cents']) }}</div>
        </div>
        <div class="rounded-lg border border-border p-5">
            <div class="text-sm text-muted-foreground">{{ __('Total QPIP (employee + employer)') }}</div>
            <div class="font-mono text-2xl font-semibold">{{ $this->money($s['total_qpip_cents']) }}</div>
        </div>
        <div class="rounded-lg border border-border p-5">
            <div class="text-sm text-muted-foreground">{{ __('Employer levies (QHSF + CNESST)') }}</div>
            <div class="font-mono text-2xl font-semibold">{{ $this->money($s['qhsf_cents'] + $s['cnesst_cents']) }}</div>
        </div>
    </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4">
                <div class="text-right text-sm text-muted-foreground">
                    <div>{{ __('Quebec payroll: :amount', ['amount' => $this->money($s['quebec_gross_cents'])]) }}</div>
                    <div>{{ __('Quebec employees in period: :n', ['n' => $s['employee_count']]) }}</div>
                </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-muted">
                <tr>
                    ...
                    <th class="px-4 py-2 text-right">{{ __('Quebec gross') }}</th>
                    <th class="px-4 py-2 text-right">{{ __('QPP') }}</th>
                    <th class="px-4 py-2 text-right">{{ __('QPIP') }}</th>
                    <th class="px-4 py-2 text-right">{{ __('Quebec tax') }}</th>
                    <th class="px-4 py-2 text-right">{{ __('QHSF + CNESST') }}</th>
            ...
            <tbody class="divide-y divide-border">
                @forelse ($this->rows as $row)
                    <tr>
                        ...
                        <td class="px-4 py-2 text-right font-mono">{{ $this->money($row['quebec_gross_cents']) }}</td>
                        <td class="px-4 py-2 text-right font-mono">{{ $this->money($row['qpp_cents']) }}</td>
                        <td class="px-4 py-2 text-right font-mono">{{ $this->money($row['qpip_cents']) }}</td>
                        <td class="px-4 py-2 text-right font-mono">{{ $this->money($row['quebec_tax_cents']) }}</td>
                        <td class="px-4 py-2 text-right font-mono">{{ $this->money($row['levies_cents']) }}</td>
                        <td class="px-4 py-2 text-right font-mono font-semibold">{{ $this->money($row['remittance_cents']) }}</td>
                    </tr>
                @empty
                    <tr><td colspan="9" class="px-4 py-8 text-center text-muted-foreground">{{ __('No posted Quebec pay runs in this period.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="mt-4 text-sm text-muted-foreground">
        <p>{{ __('Enter these totals on your TPZ-1015.R.14 statement in My Account for businesses at Revenu Québec. QPP and QPIP include both employee and employer portions; QHSF and CNESST are employer contributions.') }}</p>
    </div>
    ...
    <flux:modal name="record-remittance" class="max-w-lg">
        <form wire:submit="record" class="space-y-5">
            <div>
                ...
                <flux:subheading>{{ __(':period — :amount to Revenu Québec', ['period' => $this->selectedPeriod()['label'], 'amount' => $this->money($this->summary['remittance_due_cents'])]) }}</flux:subheading>
            </div>
