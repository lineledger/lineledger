...
class UpdateBillPaymentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $payment = $this->route('billPayment');
        $type = $payment?->payment_type ?? BillType::Vendor;
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $v) {
            ...
            $payment = $this->route('billPayment');
            $type = ($payment?->payment_type ?? BillType::Vendor)->value;
            ...
            foreach ((array) $this->input('applications', []) as $i => $app) {
                ...
                if (! $exists) {
                    $v->errors()->add("applications.$i.bill_id", 'Bill does not belong to the same contact.');
                }
            }
        });
    }
