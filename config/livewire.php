...
return [
    ...
    */
    ...
    'temporary_file_upload' => [
        // Pinned to `local`, NOT the default disk. Uploads are staged here only
        // until the app moves them to their permanent home, and several
        // consumers read the staged file through `TemporaryUploadedFile::getRealPath()`
        // (CSV list imports, chart-of-accounts import, the migration wizard,
        // bank reconciliation) — which only works on the local filesystem.
        // Where the *permanent* copy lives is a separate decision, made per
        // file kind by `filesystems.roles`.
        'disk' => env('LIVEWIRE_TEMPORARY_FILE_UPLOAD_DISK', 'local'), // Example: 'local', 's3'    | Default: 'default'
        // Staging ceiling for every Livewire upload; the per-feature app rules
        // enforce the real cap — 10 MB for transaction attachments and for
        // documents (AttachmentService::uploadRules), 100 MB for backup restores
        // (companies.restore). Sized to the largest of those. Real HTTP uploads
        // also need PHP `upload_max_filesize`/`post_max_size` AND the web server's
        // body-size limit (nginx `client_max_body_size`) set at least this high.
        'rules' => ['required', 'file', 'max:102400'], // 100 MB staging ceiling
    ...
    */
    ...
    'payload' => [
        // Raised from 1MB to 2MB so a high-resolution chart PNG (sent to the
        // exportChartPdf action as a base64 data URL) isn't rejected before the
        // action runs. Large file uploads use the separate temporary_file_upload
        // channel below, so this only affects regular component payloads.
        'max_size' => 2 * 1024 * 1024,   // 2MB - maximum request payload size in bytes
